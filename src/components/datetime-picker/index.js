  ...
  methods: {
    init (range, value) {
      if (this.data.mode === 'datetime') {
        ...
        this.setData({
          range: value || this.data.range,
          rangeArrayDisplay: range || this.data.rangeArrayDisplay
        })
      }
    },
    change (e) {
    ...
    columnChange (e) {
      if (this.data.mode !== 'datetime') {
        ...
        range[e.detail.column] = e.detail.value
        this.triggerEvent('columnchange', {...e.detail, range})
        return
      }
